...
run_slingshot <- function(D, drMethod = c("tSNE", "PCA", "diffMaps","FLOWMAP"), ...){
  ...
  data_drMethod <- switch (drMethod,
                           FLOWMAP = {
                             try(source("./TI/ANPELA_FLOWMAP.R"))
                             FLOWMAP_output_list <<- BBmisc::suppressAll(do.call(ANPELA_FLOWMAP, c(list(files = D$expr, mode = "multi"), list(...))))
...
TI <- function(D, method = c("scorpius_distSpear", "scorpius_distEucl", "scorpius_distPear", "scorpius_distManh",
                             "slingshot_tSNE", "prinCurves_tSNE", "slingshot_FLOWMAP",
                             "slingshot_PCA", "slingshot_diffMaps", "prinCurves_diffMaps"),...){
  set.seed(456)
  switch (method,
          slingshot_FLOWMAP = suppressMessages(run_slingshot(D, drMethod = "FLOWMAP",...)),
