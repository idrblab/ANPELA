  ...
  for (n in 1:nruns){
    ...
    for (lineage in 1:finalMatrix[[n]]$lineages){
      ...
      for (l in 1:ncol(as.matrix(TIres$pseudotime))){
        ...
        initial_order[,n] <- try(input_matrix[,paste0("pseudot",l)][finalMatrix[[n]]$subIndex], silent = T)
        if (class(initial_order[,n]) != "numeric") {
          initial_order[,n] <- try(input_matrix[,paste0("curve",l)][finalMatrix[[n]]$subIndex], silent = T)
        }
