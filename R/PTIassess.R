  ...
  if (save_processed_res == "one_RData") {
    ...
    table <- foreach::foreach(i = 1:length(data$AP2_pro1_frame_classTI), .options.snow = opts,
                              .packages = c("flowCore", "foreach", "dplyr", "igraph", "mclust"), .combine = rbind) %dopar% {
                                ...
                                res <- try(load.Data(data$AP2_pro1_frame_classTI[[i]], index = index, measurement.time = as.matrix(data$metadata$timepoint), TIM = TIM), silent = T)
                                ...
                                if (class(R_pvalue) != "numeric") {
                                  ...
                                  if (R_pvalue > 0.05) {
                                    ...
                                    R_pvalue <- round(1 - 20*R_pvalue, 3)
                                  }
                                }
                                rm(R)
                                ...
                                print(paste0("Rob0:",Rob0,"_",i))
                                print(paste0("Rob:",Rob,"_",i))
                                rm(Rob0)


                                # Criterion D Biological Meaning-Biological consistency
                                if (!is.null(pathwayhierarchy) && file.exists(pathwayhierarchy)) {
                                  BC <- try(suppressWarnings(Bio_con(AP2_processed_D_TI, pathwayhierarchy, nruns = 3, dr_method = TIres$dr_method, TIres = TIres)), silent = T)
                                  if (class(BC) != "numeric") {
                                    BC <- NA
                                  } else {
                                    BC <- round(BC, 3)
