ANPELA_FLOWMAP <- function (mode = c("single", "multi", "one", "static-multi"), 
                            files, var.remove = c(), var.annotate = NULL, clustering.var= NULL, 
                            cluster.numbers = 200, transform = FALSE, cluster.mode = "hclust", 
                            distance.metric = "manhattan", 
                            minimum = 2, maximum = 10, save.folder = getwd(), subsamples = 200, 
                            name.sort = FALSE, out_folder_basename = NA, downsample = FALSE, 
                            seed.X = 1, savePDFs = FALSE, graph.out = c("UMAP"), 
...
{ 
  ...
  try(source("./TI/ANPELA_FLOWMAP-function.R"))
