...
private:
  ...
  const int MAX_NODES_PER_QUAD = 1; 
...
{
  if(num_nodes <= MAX_NODES_PER_QUAD) return;
  
  list<unsigned int>::const_iterator i;
  list<unsigned int> lower_left, upper_left, lower_right, upper_right;
  ...
  for(i = nodes.begin(); i != nodes.end(); i++) 
  {
    ...
    if(fabs(x - center_x) < EPSILON) {
      x += (rand() % 2 - 0.5) * EPSILON * 1e-2;
    }
    
    if(x < center_x - EPSILON) {
      left.push_back(*i);
    } 
    else if(x > center_x + EPSILON) {
      right.push_back(*i);
    }
    else {
      (rand() % 2) ? left.push_back(*i) : right.push_back(*i);
    }
  }
  
  auto split_y = [&](const list<unsigned int>& src, list<unsigned int>& lower, list<unsigned int>& upper) {
    for(auto& node : src) {
      double y = lay(node, 1);
      
      if(fabs(y - center_y) < EPSILON) {
        y += (rand() % 2 - 0.5) * EPSILON * 1e-2;
      }
      
      if(y < center_y - EPSILON) {
        lower.push_back(node);
      } 
      else if(y > center_y + EPSILON) {
        upper.push_back(node);
      }
      else {
        (rand() % 2) ? lower.push_back(node) : upper.push_back(node);
      }
    }
  };
  
  split_y(left, lower_left, upper_left);
  
  split_y(right, lower_right, upper_right);
  
  auto create_if_not_empty = [&](list<unsigned int>& nodes_list) {
    if(!nodes_list.empty()) {
      Quadrant* q = new Quadrant(nodes_list, lay, nodes_mass);
      subQuadrants.push_back(q);
    }
  };
  
  create_if_not_empty(lower_right);
  create_if_not_empty(upper_right);
  create_if_not_empty(lower_left);
  create_if_not_empty(upper_left);
}
...
inline void Quadrant::apply_repulsion_one_sided(unsigned int i, const NumericMatrix &lay, NumericMatrix &dvec, float mass_prod, float sum_sizes, float x, float y, double krep, bool prevent_overlap)
{
  float square_dist = (lay(i, 0) - x) * (lay(i, 0) - x) +
    (lay(i, 1) - y) * (lay(i, 1) - y);
  if (square_dist < 1e-9) square_dist = 1e-9;
...
// [[Rcpp::export]]
void ANPELA_layout_forceatlas2Cpp(NumericMatrix lay, NumericVector F_att_orig, NumericVector mass, NumericVector nodes_size, 
