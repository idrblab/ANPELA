...
setwd("D:/your/working/directory/ANPELA/")
library(ANPELA)
```
...
`ANPELA` requires specified data formats (you can refer to the sample data), which include (**1**) the FCS raw data files (mandatory), (**2**) metadata, the CSV file for sample annotation (mandatory), and (**3**) the prior knowledge data file (optional). The metadata file should include the columns of "filename" and "condition" for CSI studies, and the columns of "filename" and "timepoint" for PTI studies.
...
-	“conditon” refers to the corresponding biological phenotypes of each FCS file;
-	“timepoint” refers to the physical collection time of each FCS file to indicate the collection sequence.


Reading the FCS file and metadata file is a crucial step before data processing. Let's take the MC_PTI sample data from a research about signal transduction as an example. Download MC_PTI sample data and position the files to the working directory.

```{r read, eval=FALSE}
name <- "MC_PTI_case"
datapath <- "D:/your/working/directory/ANPELA/"
```
...
```{r Getmarker_png, echo = FALSE, out.width = "70%", out.extra='style="margin-left: 50px;"'}
knitr::include_graphics("figures/Getmarker.png")
```

# Data Processing and Performance Assessment

The four core function in `ANPELA`, `FC_CSI`, `MC_CSI`, `FC_PTI`, `MC_PTI`, can accomplish data processing and performance assessment in one step, which should be used in specific condition. `FC` and `MC` denote that the raw data files are acquired from **flow cytometry (FC)** and **mass cytometry (MC)**, respectively. `CSI` and `PTI` refer to **Cell Subpopulation Identification (CSI)** and **Pseudo-time Trajectory Inference (PTI)**, respectively.
...
```{r MC_PTI, eval=FALSE}
MC_PTI(name = "MC_PTI_case", datapath = datapath, 
       index_protein = index_protein, 
       cores = floor(parallel::detectCores()/2))
```

**Note**: `ANPELA` runs as many data processing workflows as possible, but certain methods require additional input. Please refer to the detailed descriptions of specific functions in `ANPELA` *R* package. If you just want to use certain compensation, transformation, normalization and signal clean methods, you can modify them through these parameters: `compensationM`, `transformationM`, `normalizationM`, `signalcleanM`.
...
```{r process_res.png, echo = FALSE, out.width = "70%", out.extra='style="margin-left: 50px;"'}
...
```{r assess_res_file.png, echo = FALSE, out.width = "50%", out.extra='style="margin-left: 50px;"'}
...
```{r assess_res_table.png, echo = FALSE, out.width = "90%", out.extra='style="margin-left: 50px;"'}
...
```{r assess_res_figure.png, echo = FALSE, out.width = "80%", out.extra='style="margin-left: 50px;"'}
